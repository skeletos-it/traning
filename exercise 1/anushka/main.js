...
function getList(dummyTransactions) {
    ...
    for (let i = 0; i <= dummyTransactions.length; i++) {
        result = result + '<li class="_minus">' + dummyTransactions[i].text +
            '<span>' + dummyTransactions[i].amount + '</span>' +
            '</li>'
...
function addEntry() {
    ...
    dummyTransactions.push({ id: dummyTransactions.length + 1, text: text1, amount: Math.abs(amount1), minus: minus1 })
